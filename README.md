...
> [!TIP]  
> For local HTTPS with a self-signed cert, keep `STAGE=local`, set `DOMAINS` to a hostname only (e.g. `lynn.local.com -> http://api:3000`), and add a hosts entry like `127.0.0.1 lynn.local.com`. Your browser will show a warning because the cert is self-signed.
